...
impl LayoutType {
    ...
    pub fn layout_with_default_repr(&self, default_repr: Repr) -> TypeLayout {
        match self {
            LayoutType::Sized(ty) => ty.layout(default_repr),
            LayoutType::UnsizedStruct(ty) => ty.layout().into(),
            LayoutType::RuntimeSizedArray(ty) => ty.layout(default_repr).into(),
        }
    }
    ...
    pub fn contains(&self, c: RecipeContains) -> bool {
        match self {
            LayoutType::Sized(ty) => ty.contains(c),
            LayoutType::UnsizedStruct(ty) => ty.contains(c),
            LayoutType::RuntimeSizedArray(ty) => ty.contains(c),
        }
    }
}
