...
impl LayoutType {
    /// Fallibly creates a new `LayoutType` of a struct.
    ///
    ...
    pub fn struct_from_parts(
        ...
        if let Some(last_unsized) = last_unsized {
            Ok(UnsizedStruct::new(struct_name, sized_fields, last_unsized, repr)?.into())
        } else {
            SizedStruct::new(struct_name, sized_fields, repr)
                .map_err(|e| match e {
                    StructureDefinitionError::FieldNamesMustBeUnique(e) => {
                        StructFromPartsError::FieldNamesMustBeUnique(e)
                    }
                    StructureDefinitionError::MustHaveAtLeastOneField(_) => {
                        StructFromPartsError::MustHaveAtLeastOneField
                    }
                })
                .map(|s| s.into())
        }
...
impl TryFrom<LayoutType> for SizedOrArray {
    ...
    fn try_from(value: LayoutType) -> Result<Self, Self::Error> {
        match value {
            ...
            LayoutType::UnsizedStruct(_) => Err(IsUnsizedStructError),
        }
    }
}
