...
impl LayoutType<'_> {
    ...
    pub fn byte_size(&self, default_repr: Repr) -> Option<u64> {
        match self {
            LayoutType::Sized(s) => Some(s.byte_size(default_repr)),
            LayoutType::UnsizedStruct(_) | LayoutType::RuntimeSizedArray(_) => None,
        }
    }
    ...
    pub fn align(&self, default_repr: Repr) -> U32PowerOf2 {
        match self {
            LayoutType::Sized(s) => s.align(default_repr),
            LayoutType::UnsizedStruct(s) => s.align(),
            LayoutType::RuntimeSizedArray(a) => a.align(default_repr),
        }
    }
    ...
    pub fn byte_size_and_align(&self, default_repr: Repr) -> (Option<u64>, U32PowerOf2) {
        match self {
            ...
            LayoutType::UnsizedStruct(s) => (None, s.align()),
            LayoutType::RuntimeSizedArray(a) => (None, a.align(default_repr)),
        }
    }
}
...
impl SizedType<'_> {
    ...
    /// This is expensive for structs. Prefer `byte_size_and_align` if you also need the size.
    pub const fn align(&self, parent_repr: Repr) -> U32PowerOf2 { self.byte_size_and_align(parent_repr).1 }

    /// This is expensive for structs as it calculates the byte size and align by traversing all fields recursively.
    pub const fn byte_size_and_align(&self, parent_repr: Repr) -> (u64, U32PowerOf2) {
...
impl SizedStruct<'_> {
    ...
    pub const fn field_offsets(&self) -> FieldOffsetsSized<'_> {
    ...
    ///
    /// This is expensive for structs as it calculates the byte size and align by traversing all fields recursively.
    pub const fn byte_size_and_align(&self) -> (u64, U32PowerOf2) { self.field_offsets().struct_byte_size_and_align() }
}
...
impl FieldOffsets<'_> {
    pub const fn next(&mut self) -> Option<u64> {
        self.field_index += 1;
        match self.field_index - 1 < self.fields.len() {
            true => {
...
impl<'a> FieldOffsetsSized<'a> {
    ...
    pub const fn struct_byte_size_and_align(mut self) -> (u64, U32PowerOf2) {
        // Finishing layout calculations
        // advancing the iterator to the end
        while self.0.next().is_some() {}
        (self.0.calc.byte_size(), self.0.calc.align())
    }

    /// Returns the inner iterator over sized fields.
    pub const fn into_inner(self) -> FieldOffsets<'a> { self.0 }
}
...
pub struct FieldOffsetsUnsized<'a> {
    ...
    last_unsized: &'a RuntimeSizedArrayField<'a>,
}
...
impl ScalarType {
    pub const fn byte_size(&self) -> u64 {
        match self {
            ScalarType::F16 => 2,
            ScalarType::F32 | ScalarType::U32 | ScalarType::I32 | ScalarType::Bool => 4,
            ScalarType::F64 => 8,
        }
    }
    ...
    pub const fn align(&self, repr: Repr) -> U32PowerOf2 {
        match repr {
            ...
            Repr::Wgsl | Repr::WgslUniform => match self {
                ...
                // not found in the WGSL spec, but exists in GLSL
                ...
                ScalarType::Bool => U32PowerOf2::_4,
            },
        }
    }
}
...
impl Matrix {
    ...
    const fn as_vector_array(&self) -> (Vector, NonZeroU32) {
        ...
        (
            Vector {
                len: vec_len,
                scalar: self.scalar.as_scalar_type2(),
            },
            array_len,
        )
    }
...
#[allow(missing_docs)]
impl SizedArray<'_> {
    pub const fn byte_size(&self, repr: Repr) -> u64 { array_size(self.byte_stride(repr), self.len) }

    pub const fn align(&self, repr: Repr) -> U32PowerOf2 { array_align(self.element.align(repr), repr) }

    pub const fn byte_stride(&self, repr: Repr) -> u64 {
...
mod tests {
    ...
    fn test_sized_array_layout() {
        let array = SizedArray {
            element: &Vector::new(ScalarType::F32, Len::X2).to_sized_type(),
    ...
    fn test_sized_field_calculations() {
        // Test custom size
        let field = SizedField {
            name: "test_field",
            custom_min_size: Some(16),
            custom_min_align: None,
            ty: SizedType::Vector(Vector::new(ScalarType::F32, Len::X2)),
        };
        ...
        let field2 = SizedField {
            name: "test_field2",
            custom_min_size: None,
            custom_min_align: Some(U32PowerOf2::_16),
            ty: SizedType::Vector(Vector::new(ScalarType::F32, Len::X2)),
        };
    ...
    fn test_runtime_sized_array_field_align() {
        let field = RuntimeSizedArrayField {
            name: "test_array",
            array: RuntimeSizedArray::new(Vector::new(ScalarType::F32, Len::X2).to_sized_type()),
            custom_min_align: Some(U32PowerOf2::_16),
        };
    ...
    fn test_sized_struct_layout() {
        ...
        let sized_struct = SizedStruct::new("TestStruct", &fields, Repr::Wgsl);
    ...
    fn test_uniform_struct_alignment() {
        ...
        let sized_struct = SizedStruct::new("TestStruct", &fields, Repr::WgslUniform);
    ...
    fn test_unsized_struct_layout() {
        ...
        let mut unsized_struct = UnsizedStruct {
            ...
            last_unsized: RuntimeSizedArrayField::new(
                "runtime_array",
    ...
    fn test_packed_struct_layout() {
        ...
        let sized_struct = SizedStruct::new("TestStruct", &fields, Repr::Packed);
    ...
    fn test_packed_ignores_custom_min_align() {
        ...
        let fields = [
            ...
            SizedField {
                ...
                custom_min_size: None,
                custom_min_align: Some(U32PowerOf2::_16),
                ty: SizedType::Vector(Vector::new(ScalarType::F32, Len::X1)), // 4 bytes, 4-byte aligned
            },
        ];
        let s = SizedStruct::new("TestStruct", &fields, Repr::Packed);
