use crate::{
    __private::SmallVec,
    cs::{Major, Repr, SizedField, UnsizedStruct},
...
impl TypeLayout<constraint::Uniform> {
    ...
    pub fn new_layout_for(ty: impl Into<cs::SizedType>) -> Self { Self::from_sized_type(ty.into()) }


    pub fn new_layout_for_unchecked(ty: impl Into<CpuShareableType>) -> Self {
        match ty.into() {
            CpuShareableType::Sized(ty) => Self::from_sized_type(ty),
            CpuShareableType::UnsizedStruct(ty) => Self::from_unsized_struct(ty),
            CpuShareableType::RuntimeSizedArray(ty) => Self::from_runtime_sized_array(ty),
        }
    }

    fn from_sized_type(ty: cs::SizedType) -> Self {
        type_layout_internal::cast_unchecked(from_sized_type(ty, Repr::Uniform))
    }

    fn from_unsized_struct(ty: cs::UnsizedStruct) -> Self {
        type_layout_internal::cast_unchecked(from_unsized_struct(ty, Repr::Uniform))
    }

    fn from_runtime_sized_array(ty: cs::RuntimeSizedArray) -> Self {
...
impl TypeLayout<constraint::Storage> {
    pub fn new_layout_for(ty: impl Into<CpuShareableType>) -> Self {
        match ty.into() {
            CpuShareableType::Sized(ty) => Self::from_sized_type(ty),
            CpuShareableType::UnsizedStruct(ty) => Self::from_unsized_struct(ty),
            CpuShareableType::RuntimeSizedArray(ty) => Self::from_runtime_sized_array(ty),
    ...
    fn from_unsized_struct(ty: cs::UnsizedStruct) -> Self {
        type_layout_internal::cast_unchecked(from_unsized_struct(ty, Repr::Storage))
    }

    fn from_runtime_sized_array(ty: cs::RuntimeSizedArray) -> Self {
        type_layout_internal::cast_unchecked(from_runtime_sized_array(ty, Repr::Storage))
    }
...
fn from_sized_type(ty: cs::SizedType, repr: Repr) -> TypeLayout {
    let (size, align, tls) = match &ty {
        cs::SizedType::Vector(v) => (v.byte_size(), v.byte_align(), TLS::Vector(*v)),
        cs::SizedType::Atomic(a) => (
            a.byte_size(),
            a.byte_align(),
            TLS::Vector(Vector::new(a.scalar.into(), ir::Len::X1)),
        ),
        cs::SizedType::Matrix(m) => (m.byte_size(Major::Row), m.byte_align(repr, Major::Row), TLS::Matrix(*m)),
        cs::SizedType::Array(a) => (
            ...
            TLS::Array(
                Rc::new(ElementLayout {
                    ...
                    ty: from_sized_type((*a.element).clone(), repr),
                }),
                Some(a.len.get()),
            ),
        ),
...
fn from_unsized_struct(s: UnsizedStruct, repr: Repr) -> TypeLayout {
...
fn sized_field_to_field_layout(field: &SizedField, offset: u64, repr: Repr) -> FieldLayoutWithOffset {
    FieldLayoutWithOffset {
        ...
        field: FieldLayout {
            ...
            ty: from_sized_type(field.ty.clone(), repr),
        },
    }
}

fn from_runtime_sized_array(ty: cs::RuntimeSizedArray, repr: Repr) -> TypeLayout {
    TypeLayout::new(
        None,
        ty.byte_align(repr),
        TLS::Array(
            Rc::new(ElementLayout {
                ...
                ty: from_sized_type(ty.element.clone(), repr),
            }),
            None,
        ),
        Some(ty.into()),
...
impl<'a> TryFrom<&'a TypeLayout<constraint::Storage>> for TypeLayout<constraint::Uniform> {
    ...
    fn try_from(s_layout: &'a TypeLayout<constraint::Storage>) -> Result<Self, Self::Error> {
        ...
        let sized = match cpu_shareable {
            CpuShareableType::Sized(sized) => sized,
            _ => {
                return Err(UniformLayoutError::MustBeSized("wgsl", cpu_shareable.into()));
            }
        };

        let u_layout = TypeLayout::<constraint::Uniform>::new_layout_for(sized);
                    ...
                    for (i, (s_field, u_field)) in s.fields.iter().zip(u.fields.iter()).enumerate() {
                        ...
                        if s_field.rel_byte_offset != u_field.rel_byte_offset {
                            let (sized_fields, last_unsized) = match s_layout.get_cpu_shareable().unwrap() {
                                cs::CpuShareableType::Sized(cs::SizedType::Struct(s)) => (s.fields().to_vec(), None),
                                cs::CpuShareableType::UnsizedStruct(s) => {
                                    (s.sized_fields.clone(), Some(s.last_unsized.clone()))
                                }
                ...
                (TLS::Array(s_ele, _), TLS::Array(u_ele, _)) => {
                    ...
                    if s_ele.byte_stride != u_ele.byte_stride {
                        let element_ty = match u_ele.ty.get_cpu_shareable().unwrap() {
                            cs::CpuShareableType::Sized(s) => s.clone(),
                            _ => {
