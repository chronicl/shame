...
use shame::mem::{Storage, Uniform};
use shame::pipeline_kind::Render;
use shame::tf::Rgba8Unorm;
use shame::{
    BindGroupIter, BindingArray, Buffer, CpuLayout, DrawContext, EncodingGuard, GpuLayout, MipFn, ReadWrite, Texture,
};
...
impl BindlessExample {
    pub fn new(gpu: &sm::Gpu, texture_count: (u32, u32)) -> Result<Self, Error> {
        ...
        let bind_group = Self::create_bind_group(gpu, &pipeline, texture_count)?;
        Ok(Self {
            pipeline,
            texture_count,
            bind_group,
    ...
    fn create_bind_group(
        ...
        for i in tex_indices_data {
            tex_indices.push(gpu.create_buffer_init(&BufferInitDescriptor {
                label: Some(&format!("texture_indices_{i}")),
